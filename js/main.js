...
function main() {
	rl.question('Enter task (1, 2, 3): ', (task) => {
		switch (task) {
			case '1':
				task1()
				break
...
function task1() {
	rl.question('Enter string: ', (entStr) => {
		...
		console.log('Result: ' + counterAddSymbol + ' = ' + realStr.join(''))
		rl.close()
	})
}
...
function task3() {
	rl.question('Enter the number of numbers: ', (amountNumbers) => {
		const vectorOddNumbers = []

		for (let i = 0; i < amountNumbers; i++) {
			rl.question('', (entNumbers) => {
				vectorOddNumbers.push(counterOddNumbers(parseInt(entNumbers)))
				if (vectorOddNumbers.length === amountNumbers) {
					console.log(vectorOddNumbers.join(' '))
					rl.close()
				}
			})
		}
