...
function task3() {
	...
	if (numbers.length >= 1 && parseInt(numbers[0]) > 0) {
		amountNumbers = parseInt(numbers[0])

		for (let i = 0; i < amountNumbers; i++) {
			console.log('Enter numbers: ')
			let entNumbers = parseInt(prompt())
			console.log('\n')

			if (isNaN(entNumbers) || entNumbers <= 0) {
				console.log('Error')
				return
			}

			console.log(
				'Number of odd digits: ' + counterOddNumbers(entNumbers) + '\n'
			)
		}
	} else {
		console.log('Error')
